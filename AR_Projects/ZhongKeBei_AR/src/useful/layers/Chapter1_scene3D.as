package useful.layers {
	...
	//	private var orbsMaterials:Vector.<TextureMaterial>;
		...
		public function addMarker (marker:FLARMarker) :void {
			...
			sun = new Sphere(40, 20, 20);
			sun.setMaterialToAllFaces(new TextureMaterial(new EmbedManager.SunImage().bitmapData)/*orbsMaterials[0]*/);
			
			shuiXing = new Sphere(0.057 * 20 * 2.5, 5, 5);
			shuiXing.setMaterialToAllFaces(new TextureMaterial(new EmbedManager.ShuiXingImage().bitmapData));
			shuiXing.x = sun.x + 50;
			shuiXingToSunLength = shuiXing.x;
			addTrack(shuiXingToSunLength, 20);
			
			jinXing = new Sphere(0.141 * 20 * 2.5, 5, 5);
			jinXing.setMaterialToAllFaces(new TextureMaterial(new EmbedManager.JinXingImage().bitmapData));
			jinXing.x = shuiXing.x + 20;
			jinXingToSunLength = jinXing.x;
			addTrack(jinXingToSunLength, 30);
			
			earth = new Sphere(0.226 * 20 * 2.5, 10, 10);
			earth.setMaterialToAllFaces(new TextureMaterial(new EmbedManager.EarthImage().bitmapData));
			earth.x = jinXing.x + 30;
			earthToSunLength = earth.x;
			addTrack(earthToSunLength, 30);
			
			huoXing = new Sphere(0.125 * 20 * 2.5, 10, 10);
			huoXing.setMaterialToAllFaces(new TextureMaterial(new EmbedManager.HuoXingImage().bitmapData));
			huoXing.x = earth.x + 30;
			huoXingToSunLength = huoXing.x;
			addTrack(huoXingToSunLength, 50);
			
			muXing = new Sphere(1.606 * 20 * 0.8 , 20, 20);
			muXing.setMaterialToAllFaces(new TextureMaterial(new EmbedManager.MuXingImage().bitmapData));
			muXing.x = huoXing.x + 40;
			muXingToSunLength = muXing.x;
			addTrack(muXingToSunLength, 70);
			
			tuXing = new Sphere(1.317 * 20 * 0.8 , 20, 20);
			tuXing.setMaterialToAllFaces(new TextureMaterial(new EmbedManager.TuXingImage().bitmapData));
			tuXing.x = muXing.x + 50;
			tuXingToSunLength = tuXing.x;
			addTrack(tuXingToSunLength, 100);
			
			tianWuangXing = new Sphere(0.579 * 20, 15, 15);
			tianWuangXing.setMaterialToAllFaces(new TextureMaterial(new EmbedManager.TianWangXingImage().bitmapData));
			tianWuangXing.x = tuXing.x + 40;
			tianWuangXingToSunLength = tianWuangXing.x;
			addTrack(tianWuangXingToSunLength,150);
			
			haiWuangXing = new Sphere(0.561 * 20, 15, 15);
			haiWuangXing.setMaterialToAllFaces(new TextureMaterial(new EmbedManager.HaiWangXingImage().bitmapData));
			haiWuangXing.x = tianWuangXing.x + 30;
			haiWuangXingToSunLength = haiWuangXing.x;
			addTrack(haiWuangXingToSunLength, 200);
