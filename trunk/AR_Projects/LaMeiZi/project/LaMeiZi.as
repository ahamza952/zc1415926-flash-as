
package  {
	import flash.display.MovieClip;
	import flash.events.KeyboardEvent;
	import flash.events.Event;
	import flash.ui.Keyboard;
	
	import collision.Collision;
	
	public class LaMeiZi extends MovieClip{
		
		private var vx:int;
		private var vy:int;
		
		public function LaMeiZi() {
			init();
			
			//trace(player.getChildByName("basketColliderLeft").name);
		}
		
		private function init():void
		{
			//player.addChild(basketColliderLeft);
			//player.addChild(basketColliderRight);
			//player.addChild(headCollider);
			
			vx = 0;
			vy = 0;
			
			stage.addEventListener(KeyboardEvent.KEY_DOWN, onKeyDown1);
			stage.addEventListener(KeyboardEvent.KEY_UP, onKeyUp1);
			addEventListener(Event.ENTER_FRAME, onEnterFrame);
		}
		
		private function onKeyDown1(event:KeyboardEvent):void
		{
			if(event.keyCode == Keyboard.LEFT)
			{
				vx = -5;
			}
			else if(event.keyCode == Keyboard.RIGHT)
			{
				vx = 5;
			}
			if(event.keyCode == Keyboard.UP)
			{
				vy = -5;
			}
			else if(event.keyCode == Keyboard.DOWN)
			{
				vy = 5;
			}
		}
		
		private function onKeyUp1(event:KeyboardEvent):void
		{
			if(event.keyCode == Keyboard.LEFT || event.keyCode == Keyboard.RIGHT)
			{
				vx = 0;
			}
			else if(event.keyCode == Keyboard.DOWN || event.keyCode == Keyboard.UP)
			{
				vy = 0;
			}
		}
		
		private function onEnterFrame(event:Event):void
		{
			//Initialize local variables
			var playerHalfWidth:uint = player.width / 2;
			var playerHalfHeight:uint = player.height / 2;
			
			//Move the player
			player.x += vx;
			player.y += vy;
			
			//Stop player at stage edges
			if(player.x + playerHalfWidth > stage.stageWidth)
			{
				player.x = stage.stageWidth - playerHalfWidth;
			}
			else if(player.x - playerHalfWidth < 0)
			{
				player.x = 0 + playerHalfWidth;
			}
			else if(player.y + playerHalfHeight > stage.stageHeight)
			{
				player.y = stage.stageHeight - playerHalfHeight;
			}
			else if(player.y - playerHalfHeight < 0)
			{
				player.y = 0 + playerHalfHeight;
			}
		}
		
		public function checkCollisionWithPlayer(wall:MovieClip)
		{
			if(player != null)
			{
				//if(Collision.test(player.getChildByName("basketColliderLeft"), wall))
				if(player.getChildByName("basketColliderLeft").hitTestObject(wall))	
				{
					//return true;
					trace("basketColliderLeft!!!");
				}
				else if(player.getChildByName("basketColliderRight").hitTestObject(wall))	
				{
					//return true;
					trace("basketColliderRight!!!");
				}
				else if(player.getChildByName("headCollider").hitTestObject(wall))	
				{
					//return true;
					trace("headCollider!!!");
				}
			}
		}
	}
	
}
